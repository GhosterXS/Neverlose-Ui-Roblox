--[[
    Neverlose Universal Hub (LOCAL)
    1) Put source.luau in the executor workspace folder
    2) Put this hub.lua in the same folder
    3) Execute hub.lua manually (no GitHub)
]]

local function LoadNeverLoseUI()
    if type(readfile) ~= "function" then
        error("Executor needs readfile(). Put source.luau in workspace and use a filesystem executor.")
    end

    local function tryLoad(body, label)
        if type(body) ~= "string" or #body < 100 then return nil end
        -- skip if clearly not the UI library
        if not string.find(body, "NeverLose", 1, true) and not string.find(body, "CreateWindow", 1, true) then
            return nil
        end
        local fn, err = loadstring(body)
        if not fn then
            warn("[Neverlose] loadstring failed for", label, err)
            return nil, err
        end
        local ok, res = pcall(fn)
        if ok and type(res) == "table" and res.CreateWindow then
            print("[Neverlose] UI loaded from:", label)
            return res
        end
        return nil, res
    end
    ...
    local paths = {
        ...
        "scripts/source.luau",
        "autoexec/source.luau",
    }

    -- also allow custom path via getgenv
    if type(getgenv().NL_UI_PATH) == "string" then
        table.insert(paths, 1, getgenv().NL_UI_PATH)
    end
    ...
    for _, p in ipairs(paths) do
        ...
        if ok then
            local lib, err = tryLoad(body, p)
            if lib then return lib end
            lastErr = err
        end
    end

    -- scan workspace with listfiles (recursive 1-2 levels)
    local found = {}
    local function scan(dir, depth)
        depth = depth or 0
        if depth > 2 then return end
        if type(listfiles) ~= "function" then return end
        local ok, files = pcall(listfiles, dir or "")
        if not ok or type(files) ~= "table" then
            ok, files = pcall(listfiles, dir or ".")
        end
        if not ok or type(files) ~= "table" then return end
        for _, f in ipairs(files) do
            local name = string.lower(tostring(f))
            if string.find(name, "source%.luau") or string.find(name, "source%.lua") then
                table.insert(found, f)
            elseif depth < 2 and not string.find(name, "%.") then
                -- might be a folder
                pcall(function() scan(f, depth + 1) end)
            end
        end
    end
    pcall(function() scan("") end)
    pcall(function() scan(".") end)
    ...
    for _, f in ipairs(found) do
        ...
        if ok then
            local lib, err = tryLoad(body, f)
            if lib then return lib end
            lastErr = err
        end
    end

    -- print what we can see to help the user
    print("[Neverlose] source.luau NOT FOUND")
    print("[Neverlose] Tried paths:")
    for _, p in ipairs(paths) do
        local exists = false
        pcall(function()
            if isfile then exists = isfile(p) end
        end)
        print("  -", p, exists and "EXISTS" or "missing")
    end
    if #found > 0 then
        print("[Neverlose] listfiles candidates:")
        for _, f in ipairs(found) do print("  -", f) end
    else
        print("[Neverlose] listfiles found no source.luau")
        if type(listfiles) == "function" then
            local ok, files = pcall(listfiles, "")
            if ok and type(files) == "table" then
                print("[Neverlose] Files in workspace root:")
                for i, f in ipairs(files) do
                    if i <= 30 then print("  -", f) end
                end
            end
        end
    end

    error("Put source.luau in the SAME folder as hub.lua (executor workspace). Error=" .. tostring(lastErr))
end

-- Soft reset previous window only
pcall(function()
    local w = getgenv().NL_WINDOW
    if w and w.Signal then
        w.Signal:SetValue(false)
    end
end)
pcall(function()
...
do
    ...
    if not ok or not res then
        warn("[Neverlose] UI load failed:", res)
        return
    end
    NeverLose = res
    ...
    getgenv().NL_UI = NeverLose
    -- Guarantee ScreenGui parent
    ...
    if sg then
        ...
        print("[Neverlose] ScreenGui parent =", sg.Parent)
    else
        warn("[Neverlose] ScreenGui is nil after UI load")
    end
end
...
local function KillOldNeverlose()
    pcall(function()
        ...
        -- (cleanup of other ScreenGuis disabled — was destroying the UI)
        getgenv().NL_ALLOW_UI = false
        getgenv().NL_INTRO_DONE = false
task.delay(3, function() getgenv().NL_INTRO_DONE = true end)
    end)
end
KillOldNeverlose()
...
-- No gate. Library may auto-open; we re-open after intro.
pcall(function()
    local oldToggle = Window.ToggleInterface
    if type(oldToggle) == "function" then
        function Window:ToggleInterface(...)
            oldToggle(self, ...)
            local open = false
            pcall(function()
                open = self.Signal and self.Signal:GetValue()
            end)
...
-- After intro: force menu visible (manual / local)
task.spawn(function()
    local t0 = os.clock()
    while not getgenv().NL_INTRO_DONE and (os.clock() - t0) < 2.5 do
        task.wait(0.05)
    end
    getgenv().NL_INTRO_DONE = true
    task.wait(0.08)
    ...
    local function show()
        local sg = NeverLose.ScreenGui
        if not sg then
            warn("[Neverlose] No ScreenGui")
            return
        end
        sg.Enabled = true
        if not sg.Parent then
            sg.Parent = (gethui and gethui()) or (get_hidden_gui and get_hidden_gui()) or game:GetService("CoreGui")
        end
        ...
        pcall(function()
            if type(Window.SetRender) == "function" then
                Window.SetRender(Window, true)
            end
        end)

        -- Fix off-screen / invisible frames
        for _, child in ipairs(sg:GetChildren()) do
            if child:IsA("Frame") then
                local big = (child.Size.X.Offset >= 400) or (child.Size.Y.Offset >= 300) or (child.AbsoluteSize.X >= 400)
                if big then
                    child.Visible = true
                    child.Parent = sg
        ...
        menuOpen = true
        ApplyMenuMouse(true)
        print("[Neverlose] MENU OPEN | parent=", tostring(sg.Parent), "children=", #sg:GetChildren())
    end

    local ok, err = pcall(show)
    if not ok then warn("[Neverlose] show error:", err) end
